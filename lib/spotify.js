  ...
  try {
    ...
    const proto = `
      ...
      message EntityCanvazRequest {
        repeated TrackUri tracks = 1;
        message TrackUri { string track_uri = 1; }
      }
      message Canvas {
        string id = 1;
        string canvas_url = 2;
    ...
    const RequestType  = root.lookupType('EntityCanvazRequest');
    const ResponseType = root.lookupType('EntityCanvazResponse');

    const encoded = RequestType.encode(
      RequestType.create({ tracks: [{ track_uri: trackUri }] })
    ).finish();
    ...
    return decoded?.canvases?.[0]?.canvasUrl || null;
