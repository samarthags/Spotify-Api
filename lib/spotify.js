...
// ── Client token (needed for internal Spotify endpoints) ──
async function getClientToken() {
  try {
    const res = await fetch('https://clienttoken.spotify.com/v1/clienttoken', {
      ...
      body: JSON.stringify({
        client_data: {
          client_version: '1.2.13.477.ga4363ded',
...
function encodeCanvasRequest(trackUri) {
  const inner = writeString(1, trackUri);
  return writeEmbedded(1, inner);
}
...
export async function getCanvasUrl(trackUri, access_token) {
  try {
    ...
    if (!clientToken) {
      console.error('Canvas: failed to get client token');
      return null;
    }
    ...
    const res = await fetch('https://spclient.wg.spotify.com/canvaz-cache/v0/canvases', {
      ...
      headers: {
        ...
        'spotify-app-version': '1.2.13.477.ga4363ded',
